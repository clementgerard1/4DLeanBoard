...
export default {
	...
	computed : {
		...
		transform : function(){
			...
			const y = (scssVariables.playerHeight.replace("px", "") * 0.35);
	...
	<g class="milestonePlayer" v-bind:class="milestoneClass">
		<g v-if="done" v-bind:transform="transform" filter="url(#filter0_d_playermilestonedone)">
			...
			<rect x="4.12132" y="22.5" v-bind:width="size2" v-bind:height="size2" rx="1.5" transform="rotate(-45 4.12132 22.5)" stroke="url(#paint0_linear_milestone)" stroke-width="3"/>
			<text fill="black" xml:space="preserve" style="white-space: pre" font-family="Oswald" font-size="1em" font-weight="500" letter-spacing="0em"><tspan x="-80" y="-10" v-html="milestone.getName()"></tspan></text>
		</g>
		<g v-else="done" v-bind:transform="transform" class="playermilestone" filter="url(#filter0_d_ms_player)">
			<rect x="2" y="22.5" v-bind:width="size" v-bind:height="size" rx="3" transform="rotate(-45 2 22.5)" fill="url(#paint0_linear_milestone2)"/>
			<rect x="4.12132" y="22.5" v-bind:width="size2" v-bind:height="size2" rx="1.5" transform="rotate(-45 4.12132 22.5)" stroke="white" stroke-width="3"/>
			<text fill="black" xml:space="preserve" style="white-space: pre" font-family="Oswald" font-size="1em" font-weight="500" letter-spacing="0em"><tspan x="-80" y="-10" v-html="milestone.getName()"></tspan></text>
		</g>
	</g>`,
}